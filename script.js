// ═══════════════════════════════════════════════════════════
// Kumon RRL Online Library - FIXED & PRODUCTION READY
// ═══════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════
// DIAGNOSTICS: Catch file:// and API key issues early
// ═══════════════════════════════════════════════════════════
function checkEnvironment() {
    if (window.location.protocol === 'file:') {
        console.error('❌ CRITICAL: Running from file:// blocks API requests (CORS).');
    ...
    if (!JSONBIN_CONFIG.API_KEY || !JSONBIN_CONFIG.API_KEY.startsWith('$2a$')) {
        console.error('❌ API Key must be a JSONBin Master Key (starts with $2a$)');
...
// 📱 Safe image compression using FileReader (handles iOS HEIC/Android WebP)
...
// 🔧 FIXED: Hard timeout, payload limit, explicit 403/401 handling
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (response.status === 400) userMsg = 'Invalid data format';
                ...
                if (window.location.protocol === 'file:') userMsg = 'Forbidden: Run on HTTP server (not file://)';
                else userMsg = 'Forbidden: Check Master Key & Bin permissions';
            }
