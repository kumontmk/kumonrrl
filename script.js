...
async function searchBookCover() {
  ...
  try {
    ...
    let coverUrl = await searchCoverFromGoogleBooks(title, author);
...
async function searchCoverFromGoogleBooks(title, author) {
  try {
...
async function setCoverFromUrl(url) {
  try {
    ...
    console.error('Failed to set cover from URL:', error);
    throw error;
  }
}
...
function openAddBookModal() {
  if (!isAdmin) { openLoginModal(); return; }
  ['newBookTitle','newBookAuthor','newBookGenre','newBookLocation','newBookRRL','newBookCover','barcode-reader'].forEach(id => {
    const el = document.getElementById(id); if (el) el.value = '';
  });
  document.getElementById('barcode-reader').style.display = 'none';
  stopBarcodeScanner();
